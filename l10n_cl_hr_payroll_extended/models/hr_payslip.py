...
#                            ''' %(rec.id))
            ...
            for slip_rec in rec.slip_ids:
                payslip_data = {'employee_name': slip_rec.employee_id.name or '',
                                'employee_mail': slip_rec.employee_id.work_email or '',
                                'employee_bank_acc': slip_rec.employee_id.bank_account_id.acc_number or '',
                                'employee_bank': slip_rec.employee_id.bank_account_id.bank_id.name or '',
            ...
            for payslip_data in all_payslip_data:
                if counter == 0:
                    ...
                    from_cell = xl_rowcol_to_cell(row, col)
                    to_cell = xl_rowcol_to_cell(row, col)
                    worksheet.write(str(from_cell) + ':' + str(to_cell), 'CORREO', header_rule_format_left)
                    col += 1

                    from_cell = xl_rowcol_to_cell(row, col)
                    to_cell = xl_rowcol_to_cell(row, col)
                    worksheet.write(str(from_cell) + ':' + str(to_cell), 'NRO. CUENTA', header_rule_format_left)
                    col += 1

                    from_cell = xl_rowcol_to_cell(row, col)
                    to_cell = xl_rowcol_to_cell(row, col)
                    worksheet.write(str(from_cell) + ':' + str(to_cell), 'BANCO', header_rule_format_left)
                    col += 1
                ...
                from_cell = xl_rowcol_to_cell(row, col)
                to_cell = xl_rowcol_to_cell(row, col)
                worksheet.write(str(from_cell) + ':' + str(to_cell), payslip_data.get('employee_mail'), emp_data_format)
                col += 1

                from_cell = xl_rowcol_to_cell(row, col)
                to_cell = xl_rowcol_to_cell(row, col)
                worksheet.write(str(from_cell) + ':' + str(to_cell), payslip_data.get('employee_bank_acc'), emp_data_format)
                col += 1

                from_cell = xl_rowcol_to_cell(row, col)
                to_cell = xl_rowcol_to_cell(row, col)
                worksheet.write(str(from_cell) + ':' + str(to_cell), payslip_data.get('employee_bank'), emp_data_format)
                col += 1
